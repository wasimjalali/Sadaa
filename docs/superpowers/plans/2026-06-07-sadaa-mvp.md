...
**Conventions for every task:** run tests with `make test` from the repo root (NOT bare `swift test`; this machine has Command Line Tools without Xcode.app, and the Makefile injects the framework paths Swift Testing needs). Commit messages are conventional commits. After each task, the build must be green: `swift build && make test`.

**Environment note (decided during Task 0):** XCTest is unavailable without Xcode.app, so all tests use the Swift Testing framework instead. Plan tasks show test code in XCTest syntax; translate mechanically while preserving identical coverage:

- `final class FooTests: XCTestCase` -> `@Suite struct FooTests` (or `final class` when deinit-based cleanup is needed)
- `func testBar()` -> `@Test func bar()`
- `XCTAssertEqual(a, b)` -> `#expect(a == b)`; `XCTAssertTrue(x)` -> `#expect(x)`
- `XCTAssertNil(x)` -> `#expect(x == nil)`; `XCTAssertNotNil(x)` -> `#expect(x != nil)`
- `XCTFail("m")` -> `Issue.record("m")`
- `setUp()` -> `init()`; `tearDown()` -> `deinit` (class suites) or `defer` inside the test
- async tests: `@Test func bar() async throws` works directly
- "expected throw" checks: `do { ...; Issue.record("expected error") } catch { ... }` or `#expect(throws:)`
